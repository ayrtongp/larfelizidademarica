...
export default async function handler(req: NextApiRequest, res: NextApiResponse,) {
  ...
  const mainCollection = db.collection('categorias')
    ...
    case 'POST':
      ...
      if (req.query.type == 'new') {
        try {
          ...
          const requiredFields = ['cod_categoria', 'nome_categoria', 'descricao'];
          const missingFields = requiredFields.filter(field => !data[field]);
          const alreadyExists = await mainCollection.findOne({ cod_categoria: dataFields.cod_categoria })

          if (missingFields.length > 0) {
            return res.status(400).json({ error: `Campos obrigatórios ausentes: ${missingFields.join(', ')}` });
          }
          else if (alreadyExists) {
            return res.status(400).json({ message: `Categoria já cadastrada: ${dataFields.cod_categoria} na data ${alreadyExists.createdAt}.`, method: 'POST', });
          }
          else {
