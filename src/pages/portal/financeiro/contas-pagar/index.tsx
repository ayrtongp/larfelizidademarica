...
export default function ContasPagarPage() {
  ...
  const fetchTitulos = useCallback(async () => {
    setLoading(true);
    try {
      const data = await S_financeiroTitulos.getAll({ tipo: 'pagar' });
      setTitulos(data);
    } catch (err) {
      console.error(err);
    } finally {
      ...
      <PortalBase>
        <div className="col-span-full p-4">
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              <h1 className="text-2xl font-semibold text-gray-800">Contas a Pagar</h1>
              <p className="text-sm text-gray-500">Gerencie os títulos a pagar</p>
            </div>
            <Button_M3
              label="+ Novo Título"
              ...
              <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between p-4 border-b">
                  <h2 className="text-lg font-semibold text-gray-800">
                    {editingTitulo ? 'Editar Título' : 'Novo Título a Pagar'}
                  </h2>
                  <button
