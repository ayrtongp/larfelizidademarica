...
const TabelaPadrao = ({ id, resultData, arrayHeaderNames, arrayRowsNames, handlePageChange, onRowClick, esconderPaginacao = false }: Props) => {
  ...
  useEffect(() => {
    setData(resultData)
    setHeaderNames(arrayHeaderNames)
    setRowNames(arrayRowsNames)
  }, [resultData])
    ...
    <div className="overflow-x-auto">
      ...
      <table id={id} className="min-w-full bg-white border border-gray-300">
        <thead className="bg-gray-100">
          <tr>
            {headerNames.map((row, index) => (
              index != 0 ? (
                <th key={index} className="py-2 px-4 border-b">{row}</th>
              ) : (
                <th key={index} className="hidden py-2 px-4 border-b">{row}</th>
              )
            ))}
          </tr>
        </thead>
        <tbody>
          {Array.isArray(data) && data.length > 0 ? (
            data.map((row: any, index) => (
              <tr onClick={() => onRowClick(row._id)} key={index} className={`cursor-pointer hover:bg-green-300 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white hover:bg-gray-100'}`}>
                {rowNames.map((nome, ind) => (
                  ind != 0 ? (
                    <td key={ind} className="py-2 px-4 border-b whitespace-nowrap">{row[nome]}</td>
                  ) : (
                    <td key={ind} className="hidden py-2 px-4 border-b">{row[nome]}</td>
                  )
                ))}
              </tr>
            ))
