...
const PerfilFotoUpload = () => {
  ...
  const handleUpload = async () => {
    try {
      ...
      const res = await fetch(`/api/Controller/UsuarioController?id=${getUserID()}`, {
        method: 'PUT',
      ...
      <div className="m-2 flex items-center justify-center flex-col border shadow-md border-black">
        ...
        <div className='p-2'>
          <div className='mb-4'>
            ...
            <input onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' name='oldPass' type="text" />
          </div>
          <div className='mb-4'>
            <label className='block text-gray-600 text-sm font-bold mb-1' htmlFor="newPass">Nova senha:</label>
            <input onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' name='newPass' type="text" />
          </div>
          <div className='mb-4'>
            <label className='block text-gray-600 text-sm font-bold mb-1' htmlFor="repPass">Confirme a senha:</label>
            <input onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' name='repPass' type="text" />
          </div>
