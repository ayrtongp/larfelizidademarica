      ...
      <div className='text-left px-3'>
        ...
        <ul className='flex flex-col gap-2'>
          {listaHistorico.length > 0 && listaHistorico.map((historico: any, index: number) => (
            <li key={index} className={`${historico.quantidade > 0 ? 'text-green-600' : 'text-red-500'}`}>{formatDateBRHora(historico.createdAt)} - {historico.nomeUsuario}  {historico.quantidade > 0 ? 'adicionou' : 'removeu'} ({historico.quantidade}) {historico.nome_insumo}</li>
          ))}
