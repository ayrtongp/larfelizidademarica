...
import { T_DadosBancariosPrestador } from '@/types/T_prestadoresServico';
...
interface Props {
  ...
  onUpdate: (dadosBancarios: T_DadosBancariosPrestador) => void;
}
...
const Tab_DadosBancarios: React.FC<Props> = ({ prestadorId, dadosBancarios, onUpdate }) => {
  const [form, setForm] = useState<T_DadosBancariosPrestador>({ ...dadosBancarios });
  ...
  const handleChange = (field: keyof T_DadosBancariosPrestador, value: string) => {
  ...
  const handleSalvar = async () => {
    try {
      setSaving(true);
      await S_prestadoresServico.updateBancarios(prestadorId, form);
      onUpdate(form);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" value={form.agencia ?? ''} onChange={(e) => handleChange('agencia', e.target.value)}
            className={inputClass} placeholder="Ex: 1234-5" />
        </div>
        <div>
          <label className={labelClass}>Conta</label>
          <input type="text" value={form.conta ?? ''} onChange={(e) => handleChange('conta', e.target.value)}
            className={inputClass} placeholder="Ex: 12345-6" />
        </div>
        <div>
